...
resource "azurerm_network_interface_backend_address_pool_association" "public_lb_bootstrap" {
    ...
    ip_configuration_name   = "${var.nic_name}"
}
...
resource "azurerm_virtual_machine" "bootstrap" {
    ...
    storage_os_disk {
        name              = "${var.cluster_name}-bootstrap_OSDisk" # os disk name needs to match cluster-api convention
        caching           = "ReadWrite"
    ...
    os_profile {
        computer_name  = "${var.cluster_name}-bootstrap-vm"
        admin_username = "core"
